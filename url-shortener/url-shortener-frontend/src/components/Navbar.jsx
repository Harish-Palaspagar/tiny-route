  ...
  return (
    <div className="h-16 bg-custom-gradient z-50 flex items-center sticky top-0">
      <div className="lg:px-14 sm:px-8 px-4 w-full flex justify-between items-center">
        {/* Branding Section */}
        <Link to="/">
          <h1 className="font-extrabold text-3xl text-white italic sm:mt-0 mt-2 transition-transform duration-300 hover:scale-105">
            TinyRoute
          </h1>
        </Link>
        ...
        <ul
          className={`flex sm:gap-10 gap-4 sm:items-center sm:mt-1 sm:pt-0 pt-3 text-slate-800 sm:static absolute left-0 top-[62px] sm:shadow-none shadow-md ${navbarOpen ? "h-fit sm:pb-0 pb-5" : "h-0 overflow-hidden"
            } transition-all duration-300 sm:h-fit sm:bg-none bg-custom-gradient sm:w-fit w-full sm:flex-row flex-col px-4 sm:px-0`}
        >
          <li className="hover:text-btnColor font-semibold transition-colors duration-200">
            <Link
        ...
        >
          {navbarOpen ? (
            <RxCross2 className="text-white text-3xl transform transition-transform duration-200 hover:scale-105" />
          ) : (
            <IoIosMenu className="text-white text-3xl transform transition-transform duration-200 hover:scale-105" />
          )}
        </button>
      </div>
    </div>
